...
paramlist <- list(
  ...
  vCropName = list(
    label = "Crop",
    value = "winter wheat",
    options = c("winter wheat", "spring wheat")
  )

)

plotlist <- list(
  "standard" = simplaceUtil::plotScalarOutput,
  "boxplot" = function(data, x, y, simids, from, to) {
      if(length(y)==1 && is.numeric(data[[y]])) {
        boxplot(formula(paste(y,"~",x)),data=data)
      }
  }
)
...
simplaceUtil::runSimplaceCustomGuiApp(
  simplacedirs, solution, projects, 
  plotlist=plotlist, paramlist = paramlist)
